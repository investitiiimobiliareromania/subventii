...
export default function ResourcesPage() {
  ...
  const categories = ["Toate", "Ghid Solicitant", "Formular APIA", "Model Plan de Afaceri", "Contract Model"];
  const institutions = ["Toate", "APIA", "AFIR", "MADR", "AFM"];
      ...
      <main className="flex-1 py-10">
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          ...
          <div className="mb-8 border-b border-slate-200 pb-6">
            <span className="text-xs font-bold uppercase tracking-wider text-emerald-800">Biblioteca Oficială de Documente Tipizate</span>
            <h1 className="mt-1 text-3xl font-extrabold text-slate-900 sm:text-4xl">
              Ghiduri PDF, Modele de Plan de Afaceri &amp; Formulare Tipizate
            </h1>
            <p className="mt-2 text-sm text-slate-600 max-w-3xl leading-relaxed">
              Descarcă gratuit ghidurile solicitantului în format PDF, modele editabile de plan de afaceri, adeverințe de primărie, contracte-cadru de arendare și machete de calcul financiar pentru APIA, AFIR, MADR și AFM.
            </p>
          </div>
          ...
          <div className="mb-8 space-y-4 rounded-2xl border border-slate-200 bg-slate-50/70 p-5">
            <div>
              <input
                type="text"
                placeholder="Caută în ghiduri, adeverințe, contracte (ex: arendă, DR-30, adeverință)..."
                value={searchQuery}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2">
              ...
              <div className="space-y-1.5">
                <span className="block text-[11px] font-bold uppercase tracking-wider text-slate-500">Instituție</span>
                <div className="flex flex-wrap gap-1.5">
                  {institutions.map((i) => (
                    <button
                      key={i}
              ...
              filteredResources.map((res) => (
                <div key={res.id} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-xs flex flex-col justify-between hover:border-emerald-200 transition-colors">
                  ...
                  <div className="border-t border-slate-100 pt-3.5 flex items-center justify-between text-xs">
                    <span className="text-[11px] text-slate-400 font-semibold">Sursă: {res.officialSource}</span>
                    <a
                      href={res.downloadUrl}
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-800 px-4 py-2 font-bold text-white hover:bg-emerald-900 shadow-xs"
                    >
